...
def gshearcorr(A, bp=None, obj=None, rspace=True, pts1=None, pts2=None, angle=np.pi/4, matrix=None, update_obj=True):
    ...
    if matrix is None:
        ...
        Qc2 = Q_corr*np.array([np.cos(angle), -np.sin(angle)]) + center[0]
        if pts1 is None:
...
#11. - global_corr
def global_corr(A, obj=None, bp=None, show=False, update_obj=True):
    """
    ...
    if bp is None:  
        ...
        bp_1 = bp
    m, data_1 = gshearcorr(A, bp_1, obj=obj, rspace=True, update_obj=update_obj)
    if show is True:
...
def quick_show(A, en, thres=5, rspace=True, saveon=False, imgName=''):
    layers = len(A)
    if rspace is False:
        imgsize = np.shape(A)[-1]
    ...
    for i in range(12):
        ...
        if rspace is True:
            ...
            ax[i//4,i%4].set_xlim(-1.2,1.2)
            ax[i//4,i%4].set_ylim(-1.2,1.2)
        stmpy.image.add_label("${}$mV".format(int(en[i*skip])), ax=ax[i//4,i%4])
    if saveon is True:
        plt.savefig("{}.png".format(imgName),dpi=200, bbox_inches='tight')
        
def quick_show_cut(A, en, qscale, thres=5, thres2=None, saveon=False, imgName=''):
...
def quick_show_single(A, en, thres=5, qscale=None, rspace=False, saveon=False, imgName=''):
    layers = len(A)
    if rspace is False:
        if qscale is None:
            imgsize = np.shape(A)[-1]
